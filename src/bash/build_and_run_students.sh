...
ssh ${indeks}@${host} "mkdir -p ${students_bash_dir} | mkdir -p ${students_target_dir} | mkdir -p ${students_source_dir} | mkdir -p ${students_test_dir}"

# kopiowanie skryptow na studentsa
scp buildAndRun/* ${indeks}@${host}:~/${students_bash_dir}

# kopiowanie kodu
scp ${source_dir}/dcl.asm ${indeks}@${host}:${students_source_dir}/dcl.asm
...
ssh ${indeks}@${host} "./${students_bash_dir}/build.sh ${students_source_dir} ${students_target_dir}"
